 ...
 * @ind: index of which the number start on the buffer
 * @buffer: buffer
 * @flags: flags
 * @width: width
...
/**
 * write_unsgnd - write a number using a buffer
 ...
 * Return: numberof printed chars
 */
int write_pointer(char buffer[], intr ind, int length,
...
buffer[i] = padd;
buffer[i] = '\0';
if (flags & F_MINUS && padd == ' ')/* Asign extrachar to left of buffer */
{
buffer[--ind] = 'x';
buffer[--ind] = '0';
if (extra_c)
